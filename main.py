...
async def add_result(result: ResultCreate, session: Session = Depends(get_db)):
    ...
    post_result = result.copy()
    team = get_team_by_anet_id(session, post_result["anet_team_id"])
    if not team:
        raise HTTPException(
            status_code=400, detail="This team does not exist in the db"
        )
    athlete = get_athlete_by_anet_id(session, post_result["anet_athlete_id"])
    if not athlete:
        raise HTTPException(
            status_code=400, detail="This athlete does not exist in the db"
        )
    meet = get_meet_by_anet_id(session, post_result["anet_meet_id"])
    if not meet:
        raise HTTPException(
    ...
    post_result.pop("grade")
    post_result.pop("first_name")
    post_result.pop("last_name")
    post_result.pop("grade")
    post_result.pop("anet_team_id")
    post_result.pop("anet_athlete_id")
    post_result.pop("anet_meet_id")
